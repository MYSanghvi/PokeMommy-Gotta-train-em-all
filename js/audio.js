...
const __memorySettings = Object.create(null);
...
function lsGet(key, fallback) {
  ...
  return Object.prototype.hasOwnProperty.call(__memorySettings, key)
    ? __memorySettings[key]
    : fallback;
}

function lsSet(key, val) {
  __memorySettings[key] = String(val);
...
function unduckBgm() {
  ...
  const start = bgmAudio.volume;
  const STEPS = 25;
  const PERIOD = 600 / STEPS;
  let step = 0;

  _unduckTimer = setInterval(() => {
    step++;
    const t = step / STEPS;
    bgmAudio.volume = Math.min(
      start + (target - start) * (1 - Math.pow(1 - t, 2)),
      1
    );
...
function playWhosThatAudio() {
  if (!soundOn) return;
  whosThatAudio.volume = sfxVolume;
